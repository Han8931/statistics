...
Suppose that $X_1,\dots,X_n$ are i.i.d. random variables with expected values $EX_i=\mu<\infty$ and variance Var$(X_i)=\sigma^2<\infty$. 

%Let $S_N = \sum_{n=1}^N X_n$ be the sum of the RVs.One can show that, as $N$ increases the distribution of this sum approaches

Then as we saw above, the sample mean and variance have $E\overline{X}=\mu$ and Var$(\overline{X})=\frac{\sigma^2}{n}$. Thus, the normalized random variable
