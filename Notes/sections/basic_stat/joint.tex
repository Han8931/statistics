...
\begin{align*}
	...
	E_Y[E[X|Y]] &= E[X]
\end{align*}
Since, $E[E[X|Y]]$ is the function of $Y$. It is also called \textit{the law of iterated expectations} or \textit{the law of total expectation}.
\begin{align*}
	\mathbb{E}_Y[\mathbb{E}[X|Y]]&= \mathbb{E}_Y\Bigg[\sum_{x}x\cdot P(X=x|Y)\Bigg]\\
	...
	&= \mathbb{E}[X]
\end{align*}
To get an intuition, consider the following example: Let $X$ be the lifetime duration of a lightbulb, and let $Y$ be the factory the lightbulb was produced in. Suppose $E[X|Y=1]=5000$ and $E[X|Y=2]=4000$, indicating that factory 1 produces longer lasting bulbs. Suppose factory 1 supplies 60\% of the lightbulbs, so $p(Y=1)=0.6$ and $p(Y=2)=0.4$. Then the expected duration of a random lightbulb is given by
\begin{align*}
	E[X] = E[X|Y=1]p(Y=1)+E[X|Y=2]p(Y=2)
\end{align*}
There is a similar formula for the variance. In particular, \textit{the law of total variance}, also called the conditional variance formula, tells us that
\begin{align*}
	V[X] = E_Y[V[X|Y]]+V_Y[E[X|Y]]
\end{align*}
To see this, let's define the conditional moments, $\mu_{X|Y}=E[X|Y]$, $s_{X|Y}=E[X^2|Y]$, and $\sigma^2_{X|Y}=V[X|Y]=s_{X|Y}-\mu^2_{X|Y}$, which are functions of $Y$. Then, we have
\begin{align*}
	V[X] &= E[X^2]-(E[x])^2 = E_Y[s_{X|Y}]-(E_Y[\mu_{X|Y}])^2\\
		 &= E_Y[\sigma^2_{X|Y}]+E_Y[\mu^2_{X|Y}]-(E_Y[\mu_{X|Y}])^2\\
		 &= E_Y[V[X|Y]]+V_Y[\mu_{X|Y}]
\end{align*}
...
% =======
The convolution can be thought as a sum of two independent RVs. For example, suppose we roll two dice. Let $y = x_1+x_2$ be the sum of the dice. We have

\begin{align*}
	p(y=2) &= p(x_1=1)p(x_2=1) = \frac{1}{36}\\
	p(y=3) &= p(x_1=1)p(x_2=2) + p(x_1=2)p(x_2=1) = \frac{2}{36}\\
	\vdots
\end{align*}


\paragraph{Example} Let $X$ and $Y$ be two independent RVs, where both are Gaussians and let $Z = X+Y$. Find the PDF of $Z$.
% >>>>>>> 51951dfa8c4c10d83e5ec9c40d9936455a43b025
...
Note that if $X\sim \mathcal{N}\left(\mu_X, \sigma_X^2\right)$ and $Y\sim \mathcal{N}\left(\mu_Y, \sigma_Y^2\right)$, then 
