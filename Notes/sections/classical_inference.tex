...
% \begin{itemize}
% 	\item Dataset: $\mathcal{X} = \{X_1, \cdots, X_N\}$, where $X_n\sim f_X(x;\thetha)$ are i.i.d.
% 	\item We want to construct an \textbf{estimator} $\hat{\Theta}$ of $\theta$ from the dataset, which is also known as \textbf{statistic}.
% 		\begin{itemize}
% 			\item An estimator $\hat{\Theta}$ is a function of the samples$X_1, \cdots, X_N$
% 			\item $\hat{\Theta} = g(X_1, \cdots, X_N)$
% 			\item $\hat{\Theta}$ is a random variable, which has PDF, CDF and so on, since it is a function of the random samples.
% 		\end{itemize}
% 	\item The confidence measures the quality of the estimator when compared to the true parameter $\theta$.
% \end{itemize}

% \section{Understanding Confidence Interval}

% \begin{itemize}
% 	\item The confidence interval is a probabilistic statement about $\hat{\Theta}$. Instead of studying $\hat{\Theta}$ as a \textit{point}, we construct an \textit{interval}. 
% 		$$\mathcal{I} = \bigg[\hat{\Theta}-\epsilon, \hat{\Theta}+\epsilon\bigg],$$
% 		for some $\epsilon$ to be determined. Note that this interval is a \textit{random interval}. If we have a different realization of $\hat{\Theta}$, we will have a different $\mathcal{I}$. We call $\hat{\Theta}$ the confidence interval for the estimator $\hat{\Theta}$.
% 	\item Given this random interval, what is the probability of $\mathcal{I}$ includes $\theta$?
% 		$$\mathbb{P}[\theta\in \mathcal{I}] = \mathbb{P}\bigg[\hat{\Theta}-\epsilon\leq\theta\leq \hat{\Theta}+\epsilon\bigg].$$
% 	\item In practice, we often set $\mathbb{P}[\theta\in \mathcal{I}]$ to be greater than a certain confidence level, say 95\%, and then we determine $\epsilon$.
% 	\item Once we have determined $\epsilon$, we can claim that with 95\% probability the interval $\bigg[\hat{\Theta}-\epsilon, \hat{\Theta}+\epsilon\bigg]$ will include the unknown parameter $\theta$.
% 	\item To make this more general, we define $1-\alpha$ as the \textbf{confidence level} for some parameter $\alpha$.
% 		$$\mathbb{P}\bigg[\hat{\Theta}-\epsilon\leq\theta\leq \hat{\Theta}+\epsilon\bigg]\geq 1-\alpha$$
% 	\item For what $\epsilon$ can we cover 95\% of the histogram of $\hat{\Theta}$?
% 	\item To find the answer, we set up the following probability inequality:
% 		$$\mathbb{P}\Large\Bigg[\frac{|\hat{\Theta}-\mathbb{E}[\hat{\Theta}]|}{\sqrt{Var[\hat{\Theta}]}}\leq \epsilon\Bigg]\geq 1-\alpha$$
% 	\item $Z=\frac{|\hat{\Theta}-\mathbb{E}[\hat{\Theta}]|}{\sqrt{Var[\hat{\Theta}]}}$
...
% By rearranging it, we can show that 
% \begin{align*}
% 	\overline{S}^2 = \frac{1}{n}\left(\sum_{k=1}^nX_k^2-n\overline{X}^2\right).
% \end{align*}

% \paragraph{Example:} Let $X_1,\dots,X_n$ be a random sample with $EX_i=\mu$, and variance Var$(X_i)=\sigma^2$. Suppose that we use the above property to estimate $\sigma^2$. Find the bias of this estimator:
% \begin{align*}
% 	B(\overline{S}^2) = E[\overline{S}^2] - \sigma^2.
% \end{align*}

% First note that 
% \begin{align*}
% 	E{\overline{X}}^2 &=\left(E\overline{X}\right)^2+\text{Var}(\overline{X})\\
% 					  &=\mu^2+\frac{\sigma^2}{n}. \quad \text{sample variance by law of large numbers}
% \end{align*}
% Thus, 

% \begin{align*}
% 	E[\overline{S}^2] &= \frac{1}{n}\left(\sum_{k=1}^nEX_k^2-nE\overline{X}^2\right)\\
% 					  &= \frac{1}{n}\left(n(\mu^2+\sigma^2) - n\left(\mu^2+\frac{\sigma^2}{n}\right)\right)\\
% 					  &= \frac{n-1}{n}\sigma^2.
% \end{align*}
