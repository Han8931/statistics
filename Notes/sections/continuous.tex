...
% \end{align*}
Since $g$ is strictly increasing, its inverse function $g^{-1}$ is well defined. That is, for each $y\in R_Y$, there exists a \textbf{unique} $x_1$ such that $g(x_1)=y$. We can write $x_1=g^{-1}(y)$.
\begin{align*}
	F_Y(y) &= P(Y\leq y)\\
		   &= P(g(X)\leq y)\\
...
Step 2: Determine the ranges of \(Y\)

The ranges of \(Y\) are derived from the transformation:
...
Combining these, we get the range of \(Y\) as \(0 \leq Y \leq 1\).

Step 3: Find the CDF of \(Y\)

The CDF of \(Y\), \(F_Y(y)\), is given by \(F_Y(y) = P(Y \leq y)\). We need to consider the two different transformations:

1. For \(0 \leq y \leq \frac{1}{4}\):
\[ Y = X^2 \]
\[ P(Y \leq y) = P(X^2 \leq y) = P(X \leq \sqrt{y}) \]
\[ F_Y(y) = P(X \leq \sqrt{y}) = \int_{0}^{\sqrt{y}} 2x \, dx \]
\[ F_Y(y) = \left[ x^2 \right]_{0}^{\sqrt{y}} = (\sqrt{y})^2 = y \]

2. For \(\frac{1}{4} < y \leq 1\):
\[ Y = 2X - 1 \]
